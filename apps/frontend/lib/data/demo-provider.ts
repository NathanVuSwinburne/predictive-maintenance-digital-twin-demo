...
export class DemoDigitalTwinProvider implements DigitalTwinDataProvider {
  ...
  async createThread(input: { userId: string; title?: string }) { const id = `demo-thread-${++this.counter}`; const thread: ChatThread = { id, title: input.title ?? "Fleet briefing", updatedAt: NOW, userId: input.userId, promptSuggestions: ["Why was this machine so noisy yesterday?", "Plot session 78 telemetry", "Compare fleet risk", "Predict failure for Process Pump 02", "Simulate Packaging Drive 01 for 60 minutes"], followUpSuggestions: ["Show latest Packaging Drive 01 values as a table", "Compare fleet risk"] }; this.threads.set(id, thread); this.messages.set(id, []); return { ...thread }; }
